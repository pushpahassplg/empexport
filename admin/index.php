<?php
// This file is part of Moodle - http://moodle.org/local/empexport/admin/
//
...
// Load courses and it's count from api.
if (is_siteadmin()) {
    $courses = get_courses();
} else {
...
foreach ($courses as $singlecourse) {
    ...
    $posturl = '/SplashService.svc/GetDashBoardDataWebAPI';
    $result = $ahc->get_api_result($posturl, $datatosend);
    $apiresultarray = $result["GetDashBoardDataWebAPIResult"];

    $singlecarddata = array("courseid" => $singlecourse->id, "coursename" => $singlecourse->fullname,
        "coursedata" => $apiresultarray);

    array_push($carddata, $singlecarddata);

}

$cardtemplatedata = (object)[
    'menulogo' => new moodle_url('/local/empexport/pix/eklavvyalogo.png'),
    'isadmin' => is_siteadmin(),
    'hometitle' => get_string('adminhometitle', 'local_empexport'),
    'dataary' => $carddata,
];
...
echo $OUTPUT->render_from_template('local_empexport/admin/home', $cardtemplatedata);
